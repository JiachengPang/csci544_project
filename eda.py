...
def label_distribution(dataset=snli_dataset):
    ...
    label_distribution.index = label_distribution.index.map(
        {0: "entailment", 1: "neutral", 2: "contradiction", -1: "unknown"}
    )
...
def sentence_length(dataset=snli_dataset):
    ...
    filtered_df["premise_length"] = filtered_df["premise"].apply(
        lambda x: len(x.split())
    )
    filtered_df["hypothesis_length"] = filtered_df["hypothesis"].apply(
        lambda x: len(x.split())
    )

    plt.figure(figsize=(12, 6))
    plt.hist(
        filtered_df["premise_length"],
        bins=30,
        alpha=0.7,
        color="blue",
...
def words_frequency(dataset=snli_dataset):
    ...
    label_word_freqs = {
        label: {
            ...
            "hypothesis": get_label_word_frequencies(
                filtered_dataset, "hypothesis", label
            ),
        }
